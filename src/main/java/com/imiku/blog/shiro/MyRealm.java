	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		if (list.size() != 0) {
			...
			SimpleAuthenticationInfo authenticationInfo = new SimpleAuthenticationInfo(username, // 用户名
					list.get(0).getPassword(), // 密码
					ByteSource.Util.bytes(username + "" + list.get(0).getCredentialsSalt()),// salt=username+salt
					getName() // realm name
			);
//			clearCachedAuthenticationInfo(authenticationInfo.getPrincipals());
	...
	public void updateAllCache(PrincipalCollection principals){
